// Using Cropper.js for post image upload (adapted from avatar-cropper.js)

document.addEventListener('DOMContentLoaded', function () {
  const addpostBtn = document.querySelector('.add-action .btn');
  ...
  const imageUpload = `
    <div class="image-upload-group">
      ...
      <div class="file-upload">
        ...
        <button type="button" class="browse-btn" id="postBrowseBtn" style="background:linear-gradient(135deg,#fcd34d,#f59e0b);color:white;border:none;padding:0.5rem 1rem;border-radius:0.5rem;cursor:pointer;">Browse Files</button>
        <input type="file" id="postImageInput" accept="image/jpeg" style="display:none;">
        <p class="upload-note" style="font-size:0.75rem;color:#80858f;margin-top:0.5rem;">Max size: 2MB</p>
      </div>
      <div id="postCropContainer" style="margin:10px 0;"></div>
      <div class="form-actions" style="display:flex;gap:1rem;padding-top:1rem;">
        <button type="button" id="postSaveBtn" class="btn-primary" style="display:none;flex:1;">Save Image</button>
      </div>
    </div>
  `;
  ...
  const browseBtn = document.getElementById('postBrowseBtn');
  const cropContainer = document.getElementById('postCropContainer');
  const saveBtn = document.getElementById('postSaveBtn');
  let cropper = null;
  ...
  input.addEventListener('change', function (e) {
    ...
    reader.onload = function (evt) {
      cropContainer.innerHTML = `<img id='postCropImg' src='${evt.target.result}' style='max-width:100%;max-height:300px;'>`;
      const img = document.getElementById('postCropImg');
  ...
  // Save cropped image (for add post: attach to form, not AJAX)
  saveBtn.addEventListener('click', function () {
    ...
    cropper.getCroppedCanvas({ width: 400, height: 400 }).toBlob(function (blob) {
      ...
      const croppedFile = new File([blob], 'post.jpg', { type: 'image/jpeg' });
      dt.items.add(croppedFile);
      fileInput.files = dt.files;
      modalForm.appendChild(fileInput);
      alert('Image ready! Proceed to create your post.');
    }, 'image/jpeg');
  });

  // Remove image
  /*removeBtn.addEventListener('click', function () {
    if (!confirm('Are you sure you want to remove the post image?')) return;
    const formData = new FormData();
    formData.append('remove_post_image', '1');
